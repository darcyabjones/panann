...
if ( params.remote_alignments ) {
    Channel
        .fromPath(params.remote_alignments, checkIfExists: true )
        .splitCsv(by: 1, sep: '\t', header: true)
        .filter { !is_null(it.name) && !is_null(it.remote_alignments) }
        .map { [it.name, file(it.remote_alignments, checkIfExists: true)] }
        .set { userAlignedRemoteProteins }

    remoteProteins = Channel.empty()

} else if ( params.remote_proteins ) {
    Channel
...
if ( params.remote_alignments ) {
    alignedRemoteProteins = userAlignedRemoteProteins
} else {
    alignedRemoteProteins = tmpAlignedRemoteProteins
}
...
process extractExonerateRemoteProteinHints {
    ...
    input:
    set val(name), file("exonerate.gff") from alignedRemoteProteins
